...
class VolatilityNormalizedTrendCPStrategy(Strategy):
    ...
    def init(self):
        ...
        self.stop_loss_pct = -95.0
        self.take_profit_pct = 97.0
    ...
    def next(self):
        ...
        self.position_size = np.clip(0.01 + 2*regime_score, 0.01, 0.9)
...
if __name__ == '__main__':
    ...
    data = prepare_data.load_data(ticker=ticker, start_date=start_date, end_date=end_date, interval='1d')
    data = prepare_data.prepare_data(data)
    ...
    print('Median cp trend sharpe ratio:', np.median(all_sharpe_ratios))
    print('Min cp trend sharpe ratio:', np.min(all_sharpe_ratios))
    print('Mean cp trend annual return:', np.mean(all_ann_returns))
    print('Buy and hold median sharpe ratio:', np.median(buy_hold_sharpe))
